...
namespace AlfabankMerchant.RestClient
{
    ...
    /// </summary>
    public class AlfabankMerchantRestClient<TConfig> : IAlfabankMerchantRawClient<TConfig>,
        IAlfabankMerchantClient<TConfig>
        where TConfig : AlfabankConfiguration
    {
            ...
            AlfabankConfiguration? configuration, CancellationToken cancellationToken = default)
        {
                ...
                if (!Uri.IsWellFormedUriString(actionUrl, UriKind.Absolute))
                {
                    if (!string.IsNullOrEmpty(configuration?.BasePath))
                        ...
                        throw new InvalidOperationException(
                            "When configuration not provided actionUrl must be an absolute URI.");
                }
                else
            ...
            catch (Exception ex)
            {
                throw new ArgumentException(
                    $"Failed to form URI for action from '{_config!.BasePath}' and '{actionUrl}'.", ex);
            }
            ...
            {
                _logger?.LogWarning("{statusCode} {statusReason} - {endpoint}",
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (string.IsNullOrEmpty(configuration.BasePath) && string.IsNullOrEmpty(_config?.BasePath))
                throw new ArgumentException("Client gateway URL not provided. Define client BasePath in configuration.",
                    nameof(configuration));
            var actionUrl = action.FindActionUrl(CLIENT_TYPE);
            if (string.IsNullOrEmpty(actionUrl))
                throw new ArgumentException($"Unable to determine URL to call for action {action.GetType().Name}.",
                    nameof(action));

            // TODO: Validate authentication
            // TODO: Validate action properties

            var queryParams = action.GetActionParams();
            if (configuration.AuthMethod.Equals(AuthMethod.TOKEN) ||
                (configuration.AuthMethod.Equals(AuthMethod.UNDEFINED) && string.IsNullOrEmpty(configuration.Login) ||
            ...
            TConfig? configuration, CancellationToken cancellationToken = default)
        {
            ...
            return CallRawAsync(actionUrl, queryParams, (AlfabankConfiguration)(configuration ?? _config!),
                cancellationToken);
        }
            ...
            where TResponse : class
        {
            ...
            if (resp.ContainsKey("errorCode") && resp["errorCode"]!.ToString() != "0")
                throw new AlfabankException(resp["errorCode"]!.Value<int>(),
                    resp["errorMessage"]?.Value<string>() ?? "");
